...
namespace NtshEngn {
	...
	private:
		...
		void updateUDP() {
			...
			int receive = recvfrom(m_socket, buffer.data(), NTSHENGN_NETWORKING_BUFFER_SIZE, 0, reinterpret_cast<sockaddr*>(&serverSockaddr), &sockaddrSize);
		...
		void updateTCP() {
			...
			if (m_connectedServer.ipAddress != "") {
				int receive = recv(m_socket, buffer.data(), NTSHENGN_NETWORKING_BUFFER_SIZE, 0);
				if ((receive != 0) && (receive != SOCKET_ERROR)) {
					// Receive data
					if (m_dataReceivedCallback) {
						m_dataReceivedCallback(buffer.data(), receive);
